...
for cycle_num in $(seq 1 "$cycles"); do
  ...
  if ! run_to_files "$record_stdout" "$record_stderr" cargo run --locked -q -p precision-cli --bin precision -- \
  ...
  if ! run_to_files "$replay_stdout" "$replay_stderr" cargo run --locked -q -p precision-cli --bin precision -- \
