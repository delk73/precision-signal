...
use xtask::board::{parse_board_descriptor, BoardDescriptor, PreferredBackend};
...
use xtask::usb::{
    ...
    plan_report, single_check_report, Check, CommandSpec, Event, Report, Status,
};

const EXIT_OK: i32 = 0;
const EXIT_FAIL: i32 = 1;
const EXIT_USAGE: i32 = 2;
const BOARD_FILE_MISSING_HINT: &str = "use --board <id> or pass a path under the repository root";
...
fn run(args: Vec<String>, repo_root: &Path) -> i32 {
...
fn run_internal(args: Vec<String>, repo_root: &Path) -> Result<RunOutcome, UsageError> {
...
fn stlink_image_compat_plan(board: &BoardDescriptor, image_path: &str) -> xtask::usb::FlashPlan {
    xtask::usb::FlashPlan {
...
fn run_interactive(command: &CommandSpec) -> Result<(), String> {
...
fn resolve_board_arg(repo_root: &Path, board_arg: &str) -> Result<PathBuf, Report> {
...
fn needs_usb_tempdir(plan: &xtask::usb::FlashPlan) -> bool {
...
fn board_artifact_name(board_arg: &str, board_path: &Path) -> String {
    if board_arg.contains('/') || board_arg.contains('\\') || board_arg.ends_with(".toml") {
        board_path
            .file_stem()
...
fn load_board(path: &Path) -> Result<BoardDescriptor, Report> {
...
fn emit_report(report: &Report, json: bool, verbose: bool) -> Result<(), String> {
    if json {
...
fn take_value(args: &mut Vec<String>, key: &str) -> Result<String, UsageError> {
    let value = take_optional_value(args, key)?;
    value.ok_or_else(|| UsageError(format!("missing required argument: {key}")))
}

fn take_optional_value(args: &mut Vec<String>, key: &str) -> Result<Option<String>, UsageError> {
...
fn ensure_empty(rest: &[String]) -> Result<(), UsageError> {
    if rest.is_empty() {
        Ok(())
    } else {
...
fn default_flash_log_path(repo_root: &Path) -> PathBuf {
    repo_root
        .join("docs")
        .join("audits")
        .join("runlogs")
...
fn discover_repo_root(cwd: &Path) -> PathBuf {
...
mod tests {
    use super::{
        discover_repo_root, resolve_board_arg, run, run_flash_execute, run_internal, EXIT_FAIL,
        EXIT_OK, EXIT_USAGE,
    };
    use std::fs;
    use std::path::Path;
...
"#,
        ...
        assert!(super::needs_usb_tempdir(&stlink_plan));
...
"#,
        ...
        assert!(!super::needs_usb_tempdir(&openocd_plan));

        // Compatibility image plan should not require tempdir.
        let compat_plan = super::stlink_image_compat_plan(&stlink, "fw.bin");
        assert!(!super::needs_usb_tempdir(&compat_plan));
    ...
    fn needs_usb_tempdir_true_for_objcopy_semantics_without_markers() {
        ...
        assert!(super::needs_usb_tempdir(&plan));
    }
