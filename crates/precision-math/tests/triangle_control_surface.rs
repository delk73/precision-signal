...
use precision_math::{DpwGain, OscState, SignalShape, DISCONTINUITY_THRESHOLD, SCALE_2_32};
...
fn phase_from_u32(phase_u32: u32) -> precision_math::Scalar {
    // SCALE_2_32 = 2^32 in I64F64; TWO_PI / SCALE_2_32 gives radians per u32 unit.
    precision_math::Scalar::from_num(phase_u32) * (math::TWO_PI / SCALE_2_32)
}
...
fn test_triangle_freeze_tick_keeps_z_constant() {
    ...
    let mut previous_output = precision_math::TriangleDPW4::tick(&mut state, warmup_phase, &gain);
    ...
    for i in 0..8192_u32 {
        ...
        let output = precision_math::TriangleDPW4::tick(&mut state, phase, &gain);
