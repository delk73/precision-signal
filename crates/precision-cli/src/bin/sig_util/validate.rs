...
use precision_math::verification::HeaderVerifier;
...
fn read_dpw4_version_from_lock(path: &Path) -> Result<String, String> {
    ...
    for line in reader.lines() {
        ...
        if t == "[[package]]" {
            if in_package && name.as_deref() == Some("precision-math") {
                if let Some(v) = version {
                    return Ok(v);
                }
                return Err("Cargo.lock package precision-math missing version".to_string());
            }
            in_package = true;
            name = None;
            version = None;
    ...
    Err("precision-math package entry not found in Cargo.lock".to_string())
}
