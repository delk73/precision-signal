...
use precision_math::{compute_stream_hash, DpwGain, Oscillator, Scalar};
...
fn synthesize_semantic_trace(intervals: &[u32]) -> SemanticTrace {
    ...
    SemanticTrace {
        nodes: vec![
            ...
            SemanticNode {
                id: "precision_math.square".to_string(),
                values: square_values,
            },
            SemanticNode {
                id: "precision_math.hash".to_string(),
                values: hash_values,
            },
        ],
    }
...
fn validate_replay_compatibility(
    ...
    require_node(trace, "precision_math.square", target, "replay")?;
    require_node(trace, "precision_math.hash", target, "replay")?;
    Ok(())
}
...
fn validate_envelope_compatibility(
    ...
    require_node(trace, "precision_math.square", target, "envelope")?;
    Ok(())
}
