...
help:
	...
	echo "  make replay-witness-check VERSION=1.9.1"
...
rpl0-witness-check:
	$(PYTHON) scripts/test_rpl0_witness.py
	$(PYTHON) scripts/test_check_replay_witness.py

replay-witness-check:
	@test -n "$(VERSION)" || { echo "FAIL: VERSION is required. Usage: make replay-witness-check VERSION=<version>"; exit 1; }
	$(PYTHON) scripts/check_replay_witness.py --version "$(VERSION)"
