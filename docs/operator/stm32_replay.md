...
Scope: this is a legacy/debug interval CSV-oriented operator procedure for
support/characterization. It is not the primary RPL0 release capture authority
and it is not the newcomer board-bringup path. Current STM32 F446 RPL0
execution capture authority is
[docs/replay/FW_F446_CAPTURE_v1.md](../replay/FW_F446_CAPTURE_v1.md), and the
active board-bringup route is [docs/VERIFICATION_GUIDE.md](../VERIFICATION_GUIDE.md).
...
- No STATE preamble
  ...
  First check for this legacy/debug path: confirm the manual reset was performed promptly after starting capture and that any `STATE,...` line appears on `/dev/ttyACM0`.
