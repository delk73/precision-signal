...
The replay subsystem records deterministic execution evidence as replay
artifacts. An artifact contains:
...
it interprets already-captured artifacts and emits deterministic divergence
descriptions.
...
- synthetic demo fixtures regenerated by checked-in generators for Demo V3,
  Demo V4, and Demo V5
- committed hardware-captured artifacts for the captured divergence
  demonstration
...
[scripts/artifact_diff.py](../../scripts/artifact_diff.py) is the comparison tool that turns two artifacts into
a deterministic explanation.

Detection proceeds in a strict sequence:

1. Parse both artifacts.
...
- `captured evidence`: the same ladder applies to real hardware-captured
  artifacts, not just synthetic fixtures.
...
to artifacts captured from real hardware executions.
